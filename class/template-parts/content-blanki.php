...
$tags = get_the_terms(get_the_ID(), 'razdel_blanki');
$tags_id_arr = array();
if (!empty($tags)) {
...
<div class="stati--page blanki--page">
   ...
   <div class="stati__content">
     <h1 class="blanki-title"><?php echo the_title() ?></h1>
      ...
      <div class="stati__description">
        <div class="stati__description-text"><?php echo do_shortcode($postMeta['tekstoviy-block'][0]); ?></div>
        <div class="blanki__images">
          <?php if ($postMeta['blanki_images'][0] != ''): ?>
            <?php $imageArr = explode(",", $postMeta['blanki_images'][0]) ?>
            <?php foreach ($imageArr as $key => $value): ?>
              <div class="preview_img preview_img--trigger"><img data-post-id="<?php echo get_the_ID() ?>" src="<?php echo wp_get_attachment_url($value) ?>"></div>
            <?php endforeach; ?>
          <?php endif ?>
        </div>
        <div class="stati__files">
          <?php if ($postMeta['downloads_files_blanki'][0] != ''): ?>
            <?php foreach (unserialize($postMeta['downloads_files_blanki'][0]) as $key => $value): ?>
              <?php
              $linkName = wp_get_attachment_url($value['download_file_blank']);
              $fileName = get_post($value['download_file_blank'])->post_title;
              ?>
        ...
        <div class="stati__description-text"><?php echo do_shortcode($postMeta['tekstoviy-block_statey1'][0]); ?></div>
        <?php if ($siteSettings['adventages_stati_archive_checker'] == 'true'): ?>
          <div class="advertising">
            <div class="advertising__wrap">
              <a target="_blank" href="<?php echo $siteSettings['adventages_stati_archive_link']?>">
                <?php echo wp_get_attachment_image($siteSettings['adventages_stati_archive_desktop'], 'full') ?>
              </a>
            </div>
          </div>
        <?php endif; ?>
        <div class="stati__description-text"><?php echo do_shortcode($postMeta['tekstoviy-block_statey2'][0]); ?></div>
        <div class="blanki-social blanki-social--bottom">
          <div class="blanki-social__column">
            <p>Для новостей малого бизнеса мы запустили специальный канал в <a href="https://t.me/busru/?roistat_visit=4959352" target="_blank">Telegram</a><br> и группы во <a href="https://vk.com/public199514248/?roistat_visit=4959352" target="_blank">Вконтакте</a> и <a href="https://www.facebook.com/groups/646503629371121/?roistat_visit=4959352" target="_blank">Фейсбуке</a>. Присоединяйтесь! Даже <a href="https://twitter.com/Class365" target="_blank">Твиттер</a> есть.</p>
          </div>
          <div class="blanki-social__column">
  ...
  <div class="stati__item-meta_wrap">
    ...
    <div class="blanki__razdel-list">
    <div class="container">
      <?php if (!empty($posts_stati)): ?>
        <div class="blanki__razdel__title"> <a target="_blank" href="<?php echo get_term_link( $tags[0]->term_id, "razdel_blanki" ); ?>"><?php print_r($tags[0]->name) ?></a></div>
          <ul class="blanki__razdel-posts">
            <?php foreach ($posts_stati as $key => $value): ?>
              <li class="blanki__razdel-item"><a target="_blank" href="<?php echo $value->guid ?>"><?php echo $value->post_title ?></a></li>
            <?php endforeach; ?>
          </ul>
      <?php endif; ?>
      <a href="/blanki-dokumentov/" target="_blank" class="zapusk__form-more zapusk__form-more--blanki">Посмотреть полный каталог бланков</a>
    </div>
    </div>
   </div>
  </div>
